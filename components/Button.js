...
function SolidButton({
  ...
  return (
    <button
      className={
        `px-3 py-2 rounded-full font-medium whitespace-nowrap ` +
        `${
          disabled
            ? 'bg-gray-100 text-gray-500 cursor-not-allowed'
            : SOLID_BUTTON_COLORS[color]
...
function TextButton({ full, onClick, children, className, color }) {
  return (
    <button
      className={
        `px-3 py-2 rounded-full font-medium hover:bg-opacity-70 hover:bg-gray-100 focus:bg-gray-100 transition-colors ` +
...
const LIGHT_BUTTON_COLORS = {
  blue: 'text-blue-700 bg-blue-50 hover:bg-blue-100 transition-all',
};
